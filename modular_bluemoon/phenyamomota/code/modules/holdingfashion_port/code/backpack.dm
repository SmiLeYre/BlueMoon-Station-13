...
/obj/item/storage/backpack/holding/proc/teleport_damage(tele_damage, force_destruction = FALSE)
	...
	switch(stability)
		...
		if(26 to 50)
			say("Cтабильность конструкции изменена исоставляет [stability]%. Крайне рекомендуется рекалибровка.")
		if(51 to 75)
			say("Cтабильность конструкции изменена и составляет [stability]%. Рекомендуется рекалибровка.")
		if(76 to 100)
			say("Cтабильность конструкции изменена исоставляет [stability]%.")
...
/obj/item/storage/backpack/holding/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/anomaly_neutralizer) && !(user.a_intent == INTENT_HELP) && loc != user)
		to_chat(user, "<span class='notice'>You starts recolabrate [src] using [I].</span>")
		if(do_after(user, 20 SECONDS, src))
			to_chat(user, "<span class='notice'>The circuitry of [I] fries from the strain of recolabrating [src]!</span>")
