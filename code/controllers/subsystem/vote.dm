...
#define ROUNDTYPE_PLAYERCOUNT_DYNAMIC_MEDIUMPOP_MAX 55
#define ROUNDTYPE_PLAYERCOUNT_DYNAMIC_HIGHPOP_MIN 51
// BLUEMOON ADD END
				...
				if(. != ROUNDTYPE_EXTENDED)
					...
					switch(length(GLOB.clients))
						...
						if(ROUNDTYPE_PLAYERCOUNT_DYNAMIC_MEDIUMPOP_MIN to ROUNDTYPE_PLAYERCOUNT_DYNAMIC_MEDIUMPOP_MAX)
							dynamic_pick = list(ROUNDTYPE_DYNAMIC_MEDIUM)

						if(ROUNDTYPE_PLAYERCOUNT_DYNAMIC_HIGHPOP_MIN to INFINITY)
							dynamic_pick = list(ROUNDTYPE_DYNAMIC_TEAMBASED, ROUNDTYPE_DYNAMIC_HARD) - last_dynamic_type
