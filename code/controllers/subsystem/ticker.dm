...
/datum/controller/subsystem/ticker/proc/PostSetup()
	...
	if(CONFIG_GET(string/new_round_ping))
		...
		send2chat("Раунд [GLOB.round_id ? "#[GLOB.round_id]:" : "в режиме"] [hide_mode ? "секретном":"[GLOB.master_mode]"] стартует[allmins.len ? "!":" без администрации!!"]", CONFIG_GET(string/chat_announce_new_game))
	setup_done = TRUE
...
/proc/send_gamemode_vote() //CIT CHANGE - adds roundstart gamemode votes
	if(SSticker.current_state == GAME_STATE_PREGAME)
		...
		if(dynamic)
			...
			vote_time = SSticker.GetTimeLeft() - 20 SECONDS) //BLUEMOON CHANGE, WAS vote_time = (CONFIG_GET(flag/modetier_voting) ? 1 MINUTES : 20 MINUTES))
