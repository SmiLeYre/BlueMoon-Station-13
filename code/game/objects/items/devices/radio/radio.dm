...
/obj/item/radio/suicide_act(mob/living/user)
	user.visible_message("<span class='suicide'>[user] starts bouncing [src] off [user.ru_ego()] head! It looks like [user.ru_who()] trying to commit suicide!</span>")
	return BRUTELOSS
...
/obj/item/radio/proc/talk_into_impl(atom/movable/M, message, channel, list/spans, datum/language/language)
	...
	*/
	...
	if (independent && (freq == FREQ_CENTCOM || freq == FREQ_CTF_RED || freq == FREQ_CTF_BLUE || freq == FREQ_DS1 || freq == FREQ_DS2 || freq == FREQ_PIRATE || freq == FREQ_TARKOFF || freq == FREQ_SOL))
